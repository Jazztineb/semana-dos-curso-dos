...
/**
 *<h1>AmazonViewer</h1>
 * AmazonViewer es un programa que permite visualizar peliculas, series con sus respectivos capitulos,
 * libros y revistas. Te permite crear reportes generales y con fecha del día.
 * <p>
 * Existen algunas reglas como que todos los elementos pueden ser visualizados o leídos a excepción
 * de las revistas, estás solo pueden ser vistas a modo de exposición sin ser leídas.
 * 
...
public class Main {
	...
	public static void showMovies() {
		...
		do {
			...
			//Iteracion de lambda declarativa.
			AtomicInteger atomicInteger = new AtomicInteger(1);
			movies.forEach(m -> System.out.println(atomicInteger.getAndIncrement() + ". " +m.getTitle() + " Visto: " + atomicInteger.getAndIncrement()+ ". " +m.isViewed()));
	...
	public static void makeReport() {
		...
		StringBuilder contentReport = new StringBuilder(); //crea cadenas que permiten mutar
		
		//Lambdas
		movies.stream()
		.filter(m -> m.getIsViewed())
		.forEach(m -> contentReport.append(movies.toString()  + "\n"));
	...
	public static void makeReport(Date date) {
		//Muestra reporte de todo lo que se ha visto por fecha.
